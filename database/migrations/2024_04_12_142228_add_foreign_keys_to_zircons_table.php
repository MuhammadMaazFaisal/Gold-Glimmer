...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::table('zircons', function (Blueprint $table) {
            $table->foreign(['product_id'], 'zircons_ibfk_1')->references(['id'])->on('products')->onUpdate('no action')->onDelete('no action');
            $table->foreign(['vendor_id'], 'zircons_ibfk_2')->references(['id'])->on('vendors')->onUpdate('no action')->onDelete('no action');
